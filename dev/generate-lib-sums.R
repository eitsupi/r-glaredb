# Copied from <https://github.com/PRQL/prqlc-r/blob/0c4b7dfdb4ced0009cd708c8e7945e5a423a0fae/dev/generate-lib-sums.R>

base_url <- "https://github.com/eitsupi/r-glaredb/releases/download/"
...
latest_released_lib_version <- gert::git_remote_ls(remote = "https://github.com/eitsupi/r-glaredb/") |>
  dplyr::pull(ref) |>
  stringr::str_subset(stringr::str_c(r"(^refs/tags/)", tag_prefix)) |>
  stringr::str_remove_all(stringr::str_c(".*", tag_prefix, r"(|(\^.*))")) |>
  numeric_version() |>
  sort() |>
  tail(1) |>
  as.character()
